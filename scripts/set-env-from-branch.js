...
if (!PROD_URL || !BACKUP_URL) {
  console.error('\n❌ Missing LOCAL_PRODUCTION_DATABASE_URL or LOCAL_BACKUP_DATABASE_URL.');
...
function getBranch() {
  try {
    ...
    console.warn('⚠️  Could not determine git branch, defaulting to "development".');
    return 'development';
  }
}
...
if (existsSync(envPath)) {
  try {
    ...
    if (current === desired.trim()) {
      console.log(`✅ .env.local already points to ${runtime} (${extractHost(chosen)})`);
      process.exit(0);
    }
  } catch { /* ignore */ }
}
...
console.log(`📝 Wrote .env.local → ${runtime.toUpperCase()} (${extractHost(chosen)})`);
